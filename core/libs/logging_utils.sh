...
LOG_CONFIG_FILE_DEFAULT="configs/logging/logging.json"
...
_logging_load_kv() {
        ...
        if command -v jq >/dev/null 2>&1; then
                ...
                kv_lines=$(python3 - <<'PY' "$config_path" 2>/dev/null || true
import json, sys
...
logging_setup_session() {
        ...
        if [[ "${LOG_TO_FILE:-0}" == 1 ]]; then
                log_info "Log 檔案: $LOG_FILE"
        else
                log_info "Log 系統啟用 (無檔案輸出)"
        fi
